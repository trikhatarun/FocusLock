...
class TimePickerViewModel : ViewModel() {
    ...
    val sleepHours: MutableLiveData<Int> = MutableLiveData<Int>().apply { value = 4 }
    val blockText: MutableLiveData<String> = MutableLiveData< String>().apply{ value = Application.instance.resources.getString(R.string.selected_sleep_time, 4,0)}
    ...
    fun getSleepTime(): Calendar {
        val cal = Calendar.getInstance()
        val it = if(sleepHours.value != null){sleepHours.value!!}else{0}
        var minute = 0
        var hour = 4
        when( it% 2){
            0-> {
